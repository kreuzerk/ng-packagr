  ...
  (ngPkg: NgPackageData): TsConfig => {
    ...
    switch (ngPkg.jsxConfig) {
      ...
      case 'react-native':
        ...
        break;
      default:
        tsConfig.options.jsx = ts.JsxEmit.None;
        break;
    }
...
export const collectTemplateAndStylesheetFiles =
  (tsConfig: TsConfig, artefacts: NgArtifacts): ts.TransformationResult<ts.SourceFile> => {
    artefacts.temp.templates = {};
    artefacts.temp.stylesheets = {};
    const collector = componentTransformer({
      templateProcessor: (sourceFile, templateUrl, templateFilePath) => {
        artefacts.temp.templates[templateFilePath] = '';
      },
      stylesheetProcessor: (sourceFile, styleUrl, styleFilePath) => {
        artefacts.temp.stylesheets[styleFilePath] = '';
      }
    });
...
export const inlineTemplatesAndStyles =
  (tsConfig: TsConfig, artefacts: NgArtifacts): ts.TransformationResult<ts.SourceFile> => {
    ...
    const transformer = componentTransformer({
      templateProcessor: (a, b, templateFilePath) => artefacts.temp.templates[templateFilePath] || undefined,
      stylesheetProcessor: (a, b, styleFilePath) => artefacts.temp.stylesheets[styleFilePath] || undefined
    });
 ...
 */
export async function ngc(ngPkg: NgPackageData, artefacts: NgArtifacts, tsConfig: TsConfig): Promise<string> {
  ...
  const ngCompilerHost = ng.createCompilerHost({
    ...
    tsHost: compilerHostFromTransformation({
      options: tsConfig.options,
      transformation: artefacts.temp.tsSourceFiles
    })
  });
